...
namespace Chart3DLib
{
    ...
    public class DrawChart
    {
            ...
            ViewAngle va, ChartLabels cl)
        {
            ...
            for (int i = 0; i < pts1.GetLength(0) - 1; i++)
            {
                for (int j = 0; j < pts1.GetLength(1) - 1; j++)
                {
                    ...
                    {
                        ii = pts1.GetLength(0) - 2 - i;
                    }
                    pta[0] = new PointF(pts1[ii, j].X, pts1[ii, j].Y);
                    pta[1] = new PointF(pts1[ii, j + 1].X, pts1[ii, j + 1].Y);
                    pta[2] = new PointF(pts1[ii + 1, j + 1].X, pts1[ii + 1, j + 1].Y);
                    pta[3] = new PointF(pts1[ii + 1, j].X, pts1[ii + 1, j].Y);
                    if (!IsHiddenLine)
                    {
                    ...
                    if (IsColorMap)
                    {
                        Color color = AddColor(cs, pts1[ii, j], zmin, zmax, ax, va, cl);
                        aPen = new Pen(color, ds.LineStyle.Thickness);
                        aPen.DashStyle = ds.LineStyle.Pattern;
                    }
            ...
            Axes ax, ViewAngle va, ChartLabels cl)
        {
            ...
            for (int i = 0; i < pts1.GetLength(0) - 1; i++)
            {
                for (int j = 0; j < pts1.GetLength(1) - 1; j++)
                {
                    ...
                    if (IsColorMap)
                    {
                        color = AddColor(cs, pts1[ii, j], zmin, zmax, ax, va, cl);
                        aPen = new Pen(color, ds.LineStyle.Thickness);
                        aPen.DashStyle = ds.LineStyle.Pattern;
                    }
                    g.DrawPolygon(aPen, pta);
            ...
            for (int i = 0; i < pts.GetLength(0); i++)
            {
                ...
                else if (va.Elevation < 0)
                {
                    jj = 0;
                    if (va.Azimuth >= -90 && va.Azimuth <= 90)
                        jj = pts.GetLength(0) - 1;
                }

                if (i < pts.GetLength(0) - 1)
                {
                    pt3[0] = new Point3(pts[i, jj].X, pts[i, jj].Y, pts[i, jj].Z, 1);
                    pt3[1] = new Point3(pts[i + 1, jj].X, pts[i + 1, jj].Y, pts[i + 1, jj].Z, 1);
                    pt3[2] = new Point3(pts[i + 1, jj].X, pts[i + 1, jj].Y, ax.ZMin, 1);
                    pt3[3] = new Point3(pts[i, jj].X, pts[i, jj].Y, ax.ZMin, 1);
                    for (int k = 0; k < 4; k++)
            ...
            for (int j = 0; j < pts.GetLength(1); j++)
            {
                ...
                if (j < pts.GetLength(1) - 1)
                {
                    pt3[0] = new Point3(pts[ii, j].X, pts[ii, j].Y, pts[ii, j].Z, 1);
                    pt3[1] = new Point3(pts[ii, j + 1].X, pts[ii, j + 1].Y, pts[ii, j + 1].Z, 1);
                    pt3[2] = new Point3(pts[ii, j + 1].X, pts[ii, j + 1].Y, ax.ZMin, 1);
                    pt3[3] = new Point3(pts[ii, j].X, pts[ii, j].Y, ax.ZMin, 1);
                    for (int k = 0; k < 4; k++)
                        pt3[k].Transform(m, chart3d, ax, cs, cl);
                    pta[0] = new PointF(pt3[0].X, pt3[0].Y);
                    pta[1] = new PointF(pt3[1].X, pt3[1].Y);
            ...
            Axes ax, ViewAngle va, ChartLabels cl)
        {
            ...
            SolidBrush aBrush = new SolidBrush(Color.White);
            Matrix3 m = Matrix3.AzimuthElevation(va.Elevation, va.Azimuth);
            Point3[,] ptsO = ds.PointArray;
            Point3[,] pts = new Point3[ptsO.GetLength(0), ptsO.GetLength(1)];
            for (int i = 0; i < ptsO.GetLength(0); i++)
            {
                for (int j = 0; j < ptsO.GetLength(1); j++)
                {
                    pts[i, j] = new Point3(ptsO[i, j].X, ptsO[i, j].Y, ptsO[i, j].Z, 1);
                }
            }
