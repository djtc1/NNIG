...
namespace NNIG_NeuralNetworkMath
{
    ...
    public class NeuralNetwork
    {
        ...
        public NeuralNetwork(int inputs, int[] layers_desc, ActivationFunction n_act)
        {
            ...
            if (inputs < 1)
                throw new Exception("PERCEPTRON : cannot build perceptron, it must have at least 1 input");
            la = new BatchBackPropagationLearningAlgorithm(this);
            ni = inputs;
            layers = new Layer[layers_desc.Length];
            layers[0] = new Layer(layers_desc[0], ni);
            for (int i = 1; i < layers_desc.Length; i++)
        ...
        public void removeLayer(int layerIndex)
        {
            if (layerIndex != layers.Length - 1)
            {
                ...
                for (int i = 0; i < layers.Length; i++)
                {
                    if (i != layerIndex)
                    {
                        temp[j] = new Layer(layers[i].N_Neurons, layers[i].N_Inputs, layers[i].F);
                        j++;
                    }
                }

                layers = new Layer[temp.Length];
                for (int i = 0; i < temp.Length; i++)
                {
                    layers[i] = new Layer(temp[i].N_Neurons, temp[i].N_Inputs, temp[i].F);
                }

                if (layerIndex == 0)
                {
                    layers[layerIndex] = new Layer(layers[layerIndex].N_Neurons, ni, layers[layerIndex].F);
                }
                else
                {
                    layers[layerIndex] = new Layer(layers[layerIndex].N_Neurons, layers[layerIndex - 1].N_Neurons, layers[layerIndex].F);
                }
            }
        }

        public void editlayer(int layerIndex, int nNeurons)
        {
            if (layerIndex != layers.Length - 1)
            {
                layers[layerIndex] = new Layer(nNeurons, layers[layerIndex].N_Inputs, layers[layerIndex].F);
                layers[layerIndex + 1] = new Layer(layers[layerIndex + 1].N_Neurons, nNeurons, layers[layerIndex + 1].F);
            }
        }

        public void insertFirstHiddenLayer(int nNeurons)
        {
            Layer[] temp = new Layer[layers.Length + 1];

            temp[0] = new Layer(nNeurons, ni);

            for (int i = 1; i <= layers.Length; i++)
            {
                temp[i] = new Layer(layers[i - 1].N_Neurons, layers[i - 1].N_Inputs, layers[i - 1].F);
            }
            ...
            layers[1] = new Layer(layers[1].N_Neurons, nNeurons, layers[1].F);
        }
